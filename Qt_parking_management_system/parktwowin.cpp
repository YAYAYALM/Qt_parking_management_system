...
#include "ui_parktwowin.h"

ParkTwoWin::ParkTwoWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::ParkTwoWin)
{
    ui->setupUi(this);

    //关联信号与槽函数
    connect(ui->btn1,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn2,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn3,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn4,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn5,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn6,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn7,&QPushButton::clicked,this,&ParkTwoWin::incar);
    connect(ui->btn8,&QPushButton::clicked,this,&ParkTwoWin::incar);
    ...
    //车牌，卡号，时间，车位
    bool ret=query.exec("create table if not exists parktwotable (name text unique,card text unique,time text,car int unique,mon int);");
    if(!ret)
    {
        perror("创建表格失败!\n");
        return;
    ...
    ret=query.exec("select * from parktwotable;");
    if(!ret)
    {
        perror("查询失败:\n");
        return;
    ...
    mytime.start(1000);
    //绑定定时器
    connect(&mytime,&QTimer::timeout,this,&ParkTwoWin::getData);
    //绑定定时器，获取卡号
    card.start(1000);
    connect(&card,&QTimer::timeout,this,&ParkTwoWin::getcard);
}

ParkTwoWin::~ParkTwoWin()
{
    mytime.stop();
    card.stop();
    delete ui;
...
void ParkTwoWin::on_rightBt_clicked()
{
    QMessageBox::information(this,"操作提示","没有下一个停车场");
}
//进入主停车场
void ParkTwoWin::on_upBt_clicked()
{
    mytime.stop();
...
//获取天气信息
void ParkTwoWin::getweather(QNetworkReply *p)
{
    //读取网址回复的应答信息
...
//退出返回
void ParkTwoWin::on_actionquit_triggered()
{
    mytime.stop();
    this->parentWidget()->show();
    card.stop();
...
void ParkTwoWin::incar()
{
    ...
    else
    {
        ...
        {
            //拼接得到完整的SQL语句
            QString cmd=QString("insert into parktwotable values(\"%1\",\"%2\",\"%3\",%4,%5);").arg(ch1).arg(ch1).arg(ch).arg(newcarnum).arg(carmon);
            QSqlQuery query(mybase);
            ...
            if(!ret)
            {
                ...
                while(query.next())
                {
                    ...
                    {
                        //拼接得到完整的SQL语句，删除
                        QString cmd=QString("delete from parktwotable where card=\"%1\";").arg(car);
                        QSqlQuery query(mybase);
                        bool ret=query.exec(cmd);
                        if(!ret)
                        {
...
//获取卡号
void ParkTwoWin::getcard()
{
    if(carnum <= 0)
    {
        QMessageBox::warning(this,"操作提示","3号车库已满");
        return;
    }
